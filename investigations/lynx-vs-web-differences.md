...
Caveats for **non-modal** overlays (e.g. toast): a full-screen first child captures touches for the app behind it. `pointer-events` errors the build (see above), and `events-pass-through`/`custom-layout` are web-platform-only overlay attributes (the native overlay API exposes only `ios-enable-swipe-back`, `level`, `mode`, `visible`) — but the general-purpose `event-through` attribute (a different mechanism, see below) does work here: set it on the backdrop and explicitly set it back to `false` on the actual interactive content (e.g. each toast card), since it's inherited. Confirmed against the native source:
...
- **iOS**: `LynxOverlayContainer.hitTest:withEvent:` (`platform/darwin/ios/lynx_xelement/overlay/LynxOverlayContainer.m`) does a real hit test and returns `nil` when the hit element has `eventThrough` set, so standard UIKit hit-testing gives the touch to whatever's behind it.
- **Android**: `LynxOverlayDialog` is a genuine separate `Dialog`/window, so passthrough is coarser — `LynxOverlayView.needHandleEvent()` (`LynxOverlayView.kt`) checks the overlay's own `event-through` plus its *direct* children's bounding boxes, not a deep per-descendant hit test. A single full-screen direct-child backdrop may make this always resolve to "handled," swallowing touches meant for nested interactive content. Not yet verified on-device — check here first if a toast/overlay is unresponsive on Android specifically.

**iOS-specific trap: `event-through` alone was not enough.** `<overlay>`'s `mode` prop (`LynxUIOverlay.m`) defaults to `LynxOverlayModeWindow` (`LynxOverlayContainer.h`) when left unset — a genuinely separate native `UIWindow` stacked above the app, not a layered sibling view. `LynxOverlayContainer.hitTest:withEvent:` returning `nil` is a same-window mechanism (it tells UIKit "not me, try my siblings/superview's other subviews"); across two separate windows, whether that `nil` actually reaches the app's main window is a different, far less certain guarantee than the plain sibling-view case. Set `mode="page"` explicitly to attach the overlay inside the same view controller/window as the rest of the page — this is what makes the `event-through` hit-test redispatch land in the single-window scenario it was verified against, and it's the piece that was actually breaking touch passthrough for `packages/ui/src/lib/components/toast/toast.ts` (setting `event-through` alone did not fix it — long-press and even a plain header button both stayed dead while a toast was visible, until `mode="page"` was added too).

Applied in `packages/ui/src/lib/components/toast/toast.ts`: the `<overlay>` gets `mode="page"`, its `h-full w-full` backdrop gets `[event-through]="true"`, and `UiToastItem`'s host overrides with `[attr.event-through]="false"`.
