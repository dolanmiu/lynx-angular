...
**Diagnostics added:**
```html
<x-text>activated:{{rOutlet.isActivated}} navigated:{{routerNavigated}} url:{{routerUrl}}</x-text>
```
...
| `packages/demo-app/src/app/app.config.ts` | App config — currently uses flat `provideRouter()` |
