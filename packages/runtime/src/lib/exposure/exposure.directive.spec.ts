...
import { LynxExposureDirective } from './exposure.directive';

// Builds a directive instance with a fake element injected via DI.
const createDirective = (nativeElement: object): LynxExposureDirective => {
  const injector = Injector.create({
    providers: [
      { provide: ElementRef, useValue: new ElementRef(nativeElement) },
    ],
  });
  return runInInjectionContext(injector, () => new LynxExposureDirective());
};
...
describe('LynxExposureDirective', () => {
