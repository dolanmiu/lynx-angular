...
```typescript title="src/app/fade-panel.ts"
...
Lynx runs Angular on a [background thread](/guide/introduction#the-dual-thread-model) (separate from the native UI) that cannot call `getComputedStyle()`. The component cannot auto-detect your CSS transition length, so you must pass it explicitly. This is the same constraint Vue Lynx has.
...
```typescript title="src/app/slide-panel.ts"
@Component({
...
```typescript title="src/app/animated-list.ts"
import { Component, signal } from '@angular/core';
