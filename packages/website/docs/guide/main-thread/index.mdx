...
:::warning
Main thread functions cannot capture closures over component state. They can only access their arguments, `MainThreadRef` values, and the global `lynx` API. If you need to communicate results back to Angular, use `LynxMainThread.runOnMainThread()` which returns a `Promise`.
:::

## Complete Example

```typescript title="src/app/main-thread-demo.ts"
