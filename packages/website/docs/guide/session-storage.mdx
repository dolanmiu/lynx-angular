...
Lynx session storage is a cross-LynxView key-value store that persists for the lifetime of the host app session. Use it to share data between multiple cards, pages, or LynxView instances. AngularLynx wraps the native API with `LynxSessionStorage` — an injectable service providing Promise-based reads, direct writes, and reactive signals.
...
No extra providers needed — `LynxSessionStorage` is `providedIn: 'root'`:

```typescript title="src/app/my.ts"
import { Component, inject } from '@angular/core';
import { LynxSessionStorage, LYNX_ELEMENTS } from '@blotch/angular-lynx'; // [!code highlight]
...
export class Example {
  readonly #sessionStorage = inject(LynxSessionStorage); // [!code highlight]
}
```
...
```typescript
const preferences = await sessionStorage.getItem<{ theme: string }>(
  'user-preferences',
);
...
```typescript title="src/app/counter.ts"
import { Component, inject } from '@angular/core';
import { LynxSessionStorage, LYNX_ELEMENTS } from '@blotch/angular-lynx';
...
```typescript title="src/app/session-storage-demo.ts"
